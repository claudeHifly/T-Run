...
public class Empty extends Item {
     ...
     * add an up arrow to indicate the action that must be performed by the
     * character in order to avoid the canyon.
     */
    public Empty(int x) {
        super(x, -1, Resources.instance().getEmpty());
        if (HomePage.demo) {
            Board.arrows.addArrowUp(x - distanceArrowCanyon, (Ground.yPosition));
        }
    }
